...
public interface MutableBytes extends Bytes {
     ...
     * @return A {@link MutableBytes} value.
     */
    static MutableBytes wrapBuffer(ByteBuffer buffer) {
        checkNotNull(buffer);
     ...
     */
    static MutableBytes wrapBuffer(ByteBuffer buffer, int offset, int size) {
        checkNotNull(buffer);
        if (size == 0) {
            return EMPTY;
        }
     ...
     * @return A {@link MutableBytes} value.
     */
    static MutableBytes wrapByteBuf(ByteBuffer byteBuf) {
     ...
     */
    static MutableBytes wrapByteBuf(ByteBuffer byteBuf, int offset, int size) {
        checkNotNull(byteBuf);
        if (size == 0) {
            return EMPTY;
        }
     ...
     * @param size       The size of the returned value.
     * @return A {@link MutableBytes} value.
     * @throws IndexOutOfBoundsException if {@code offset < 0 || (byteBuffer.limit() > 0 && offset >= byteBuffer.limit())}.
     ...
     *
     * <p>
     * Note: the resulting slice is only a view over the original value. Holding a reference to the returned slice may hold more memory than the slide represents. Use {@link #copy} on the returned slice to avoid this.
     *
     * @param i      The start index for the slice.
     * @param length The length of the resulting value.
     * @return A new mutable view over the bytes of this value from index {@code i} (included) to index {@code i + length} (excluded).
