 ...
 */
package ir.moke.tron4j.crypto.tuwenitypes;
 ...
 *
 * <p>
 * This interface makes no thread-safety guarantee, and a {@link Bytes} value is generally not thread safe. However, specific implementations may be thread-safe. For instance, the value returned by {@link #copy} is guaranteed to be thread-safe as it is immutable.
 */
public interface Bytes extends Comparable<Bytes> {
     ...
     *
     * <p>
     * Note that value is not copied and thus any future update to {@code value} within the slice will be reflected in the returned value.
     *
     * @param value  The value to wrap.
     * @param offset The index (inclusive) in {@code value} of the first byte exposed by the returned value. In other words, you will have {@code wrap(value, o, l).get(0) == value[o]}.
     * @param length The length of the resulting value.
     * @return A {@link Bytes} value that expose the bytes of {@code value} from {@code offset} (inclusive) to {@code offset + length} (exclusive).
     ...
     *
     * <p>
     * Note that the values are not copied and thus any future update to the values will be reflected in the returned value. If copying the inputs is desired, use {@link #concatenate(Bytes...)}.
     *
     ...
     */
    static Bytes wrapBuffer(ByteBuffer buffer) {
        checkNotNull(buffer);
     ...
     *
     * @param buffer The buffer to wrap.
     * @param offset The offset in {@code buffer} from which to expose the bytes in the returned value. That is, {@code wrapBuffer(buffer, i, 1).get(0) == buffer.getByte(i)}.
     * @param size   The size of the returned value.
     * @return A {@link Bytes} value.
     * @throws IndexOutOfBoundsException if {@code offset < 0 || (buffer.length() > 0 && offset >= buffer.length())}.
     * @throws IllegalArgumentException  if {@code length < 0 || offset + length > buffer.length()}.
     */
    static Bytes wrapBuffer(ByteBuffer buffer, int offset, int size) {
        checkNotNull(buffer);
        if (size == 0) {
            return EMPTY;
        }
     ...
     *
     * @param byteBuf The {@link ByteBuffer} to wrap.
     * @return A {@link Bytes} value.
     */
    static Bytes wrapByteBuf(ByteBuffer byteBuf) {
     ...
     * Note that any change to the content of the buffer may be reflected in the returned value.
     *
     * @param offset The offset in {@code byteBuf} from which to expose the bytes in the returned value. That is, {@code wrapByteBuf(byteBuf, i, 1).get(0) == byteBuf.getByte(i)}.
     * @param size   The size of the returned value.
     * @return A {@link Bytes} value.
     * @throws IndexOutOfBoundsException if {@code offset < 0 || (byteBuf.capacity() > 0 && offset >= byteBuf.capacity())}.
     * @throws IllegalArgumentException  if {@code length < 0 || offset + length > byteBuf.capacity()}.
     */
    static Bytes wrapByteBuf(ByteBuffer byteBuf, int offset, int size) {
        checkNotNull(byteBuf);
        if (size == 0) {
            return EMPTY;
        }
     ...
     *
     * @param byteBuffer The {@link ByteBuffer} to wrap.
     * @param offset     The offset in {@code byteBuffer} from which to expose the bytes in the returned value. That is, {@code wrapByteBuffer(byteBuffer, i, 1).get(0) == byteBuffer.getByte(i)}.
     * @param size       The size of the returned value.
     * @return A {@link Bytes} value.
     * @throws IndexOutOfBoundsException if {@code offset < 0 || (byteBuffer.limit() > 0 && offset >= byteBuf.limit())}.
     ...
     * @throws IllegalArgumentException if {@code value < 0} or {@code value} is too big to fit an unsigned 2-byte short (that is, if {@code value >= (1 << 16)}).
     */
     ...
     * @throws IllegalArgumentException if {@code value < 0} or {@code value} is too big to fit an unsigned 4-byte int (that is, if {@code value >= (1L << 32)}).
     */
     ...
     * @throws IllegalArgumentException if {@code value < 0} or {@code value} is too big to fit an unsigned 8-byte int (that is, if {@code value >= (1L << 64)}).
     */
     ...
     *
     * <p>
     * This method is lenient in that {@code str} may of an odd length, in which case it will behave exactly as if it had an additional 0 in front.
     *
     ...
     *
     * <p>
     * This method allows for {@code str} to have an odd length, in which case it will behave exactly as if it had an additional 0 in front.
     *
     * @param str             The hexadecimal string to parse, which may or may not start with "0x".
     * @param destinationSize The size of the returned value, which must be big enough to hold the bytes represented by {@code str}. If it is strictly bigger those bytes from {@code str}, the returned value will be left padded with zeros.
     * @return A value of size {@code destinationSize} corresponding to {@code str} potentially left-padded.
     * @throws IllegalArgumentException if {@code str} does not correspond to a valid hexadecimal representation, represents more bytes than {@code destinationSize} or {@code destinationSize < 0}.
     */
     ...
     * @param str The hexadecimal string to parse, which may or may not start with "0x".
     * @return The value corresponding to {@code str}.
     * @throws IllegalArgumentException if {@code str} does not correspond to a valid hexadecimal representation, or is of an odd length.
     */
     ...
     * @param destinationSize The size of the returned value, which must be big enough to hold the bytes represented by {@code str}. If it is strictly bigger those bytes from {@code str}, the returned value will be left padded with zeros.
     * @return A value of size {@code destinationSize} corresponding to {@code str} potentially left-padded.
     * @throws IllegalArgumentException if {@code str} does correspond to a valid hexadecimal representation, or is of an odd length.
     * @throws IllegalArgumentException if {@code str} does not correspond to a valid hexadecimal representation, or is of an odd length, or represents more bytes than {@code destinationSize} or {@code destinationSize < 0}.
     */
    ...
    /**
     * Provides the number of zero bits preceding the highest-order ("leftmost") one-bit, or {@code size() * 8} if all bits * are zero.
     *
     * @return The number of zero bits preceding the highest-order ("leftmost") one-bit, or {@code size() * 8} if all bits are zero.
     */
     ...
     *
     * @return The number of bits following and including the highest-order ("leftmost") one-bit, or zero if all bits are zero.
     */
     ...
     *
     * <p>
     * Please note that the resulting slice is only a view and as such maintains a link to the underlying full value. So holding a reference to the returned slice may hold more memory than the slide represents. Use {@link #copy} on the returned slice if that is not what you want.
     *
     ...
     * @param i      The start index for the slice.
     * @param length The length of the resulting value.
     * @return A new value providing a view over the bytes from index {@code i} (included) to {@code i + length} (excluded).
     ...
     *
     * @return A value, equals to this one, but deeply immutable and that doesn't retain any "unreachable" bytes. For performance reasons, this is allowed to return this value however if it already fit those constraints.
     */
    Bytes copy();
     ...
     *
     * @return A mutable copy of this value. This will copy bytes, modifying the returned value will <b>not</b> modify this value.
     */
     ...
     *
     * @param destination The mutable value to which to copy the bytes to, which must have the same size as this value. If you want to copy value where size differs, you should use {@link #slice} and/or {@link MutableBytes#mutableSlice} and apply the copy to the result.
     ...
     *
     * <p>
     * This is a (potentially slightly more efficient) shortcut for {@code copyTo(destination.mutableSlice(destinationOffset, this.size()))}.
     *
     * @param destination       The mutable value to which to copy the bytes to, which must have enough bytes from {@code destinationOffset} for the copied value.
     * @param destinationOffset The offset in {@code destination} at which the copy starts.
     * @throws IllegalArgumentException if the destination doesn't have enough room, that is if {@code this.size() > (destination.size() - destinationOffset)}.
     */
     ...
     *
     * @return A byte array with the same content than this value, which may or may not be the direct backing of this value.
     */
     ...
     *
     * @return This value represented as a minimal hexadecimal string (without any leading zero, except if it's valued zero or empty, in which case it returns 0x0).
     */
